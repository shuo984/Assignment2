...
void StartProgram::userLogin()
{
    ...
    if (isLogin && tempAccount.getState() == A_U_NORMAL)
    {
...
void StartProgram::resetProg()
{
    ...
    resParam << "NumAccount: " << "\t\t"  << 1 << std::endl;
    resParam << "NumRent: " << "\t\t"  << 0 << std::endl;
    resParam << "NumNotif: " << "\t\t"  << 0 << std::endl;
    resParam << "NumBookType: " << "\t\t" << 0 << std::endl;
    resParam << "NumBook: " << "\t\t" << 0 << std::endl;
    User adminUser(1,1,1,"Huỳnh Văn Lành","20/04/1998","XXX","XXX",A_U_NORMAL);
...
{
    connect(ui->regA_usnLine, &QLineEdit::textChanged,[&]{ui->regA_regButton->setEnabled((!ui->regA_usnLine->text().isEmpty()));});
    connect(ui->regA_pwLine, &QLineEdit::textChanged,[&]{ui->regA_regButton->setEnabled((!ui->regA_usnLine->text().isEmpty()));});
    connect(ui->regA_repwLine, &QLineEdit::textChanged,[&]{ui->regA_regButton->setEnabled((!ui->regA_usnLine->text().isEmpty()));});
